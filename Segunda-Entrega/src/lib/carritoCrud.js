...
class ContenedorCarrito {
    ...
    async getProductsById(idCart) {
        let docs = false
        docs = await carritoModel.findOne({ _id:ObjectId(idCart) }, { __v: 0 })
        if (docs) {
    ...
    async addProduct(idCart, idProduct) {
        ...
        const docCart = await carritoModel.findOne({ _id: ObjectId (idCart) }, {__v: 0 });
        const docProduct = await productsModel.findOne({ _id:ObjectId (idProduct) }, { __v: 0 })
    ...
    async deleteProductById(idCart, idProduct) {
        ...
        docCart = await carritoModel.findOne({ _id:ObjectId (idCart) }, { __v: 0 })
        docProduct = await productsModel.findOne({ _id:ObjectId (idProduct) }, { __v: 0 })
